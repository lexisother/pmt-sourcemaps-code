...
function parse(argv, config, propagateDefaults = true) {
  ...
  Object.keys(config).forEach(key => {
    ...
    if (option.alias != null) {
      if (typeof option.alias === "string") aliases[option.alias] = key;else if (Array.isArray(option.alias)) option.alias.forEach(alias => aliases[alias] = key);
    }
    ...
    if (currentValue == null) {
      ...
      if (Array.isArray(currentValue)) {
        ...
        if (mutuallyExclusive != null && (exclude = currentOptions[mutuallyExclusive])) {
          ...
          mergedOptions[key] = [...currentValue, ...parentValue.filter(value => !currentValue.includes(value)) // dedup
          ];
        }
      } else {
        mergedOptions[key] = currentValue;
