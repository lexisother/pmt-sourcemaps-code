...
const dynrequire = typeof __webpack_require__ === "function" ? __non_webpack_require__ : require;
...
function loadAssemblyScriptJS() {
  ...
  try {
    ...
    try {
      // `asc` on the command line (unnecessary in recent node)
      exports = dynrequire("../dist/assemblyscript.js");
    } catch (e) {
      try {
        // `asc` on the command line without dist files (unnecessary in recent node)
        ...
        if (!tsNode || !(e_ts instanceof tsNode.TSError)) {
          try {
            // `require("dist/asc.js")` in explicit browser tests
...
function loadAssemblyScriptWasm(binaryPath) {
  ...
  var {
    exports
  } = loader.instantiateSync(fs.readFileSync(binaryPath), rtrace.install({
    binaryen
  }));
...
exports.libraryFiles = exports.isBundle ? BUNDLE_LIBRARY : (() => {
  ...
  const bundled = {};
  find.files(libDir, defaultExtension.re_except_d).forEach(file => {
...
exports.definitionFiles = exports.isBundle ? BUNDLE_DEFINITIONS : (() => {
  const readDefinition = name => fs.readFileSync(path.join(__dirname, "..", "std", name, `index${defaultExtension.ext_d}`), "utf8");
  return {
...
exports.compileString = (sources, options) => {
  if (typeof sources === "string") sources = {
    [`input${defaultExtension.ext}`]: sources
  };
...
exports.main = function main(argv, options, callback) {
  ...
  let bundleMinorVersion = 0,
    bundleMajorVersion = 0,
    bundlePatchVersion = 0;
  ...
  if (opts.transform) {
    ...
    for (let i = 0, k = transformArgs.length; i < k; ++i) {
      ...
      if (!tsNodeRegistered && filename.endsWith(".ts")) {
        // ts-node requires .ts specifically
        ...
        if (sourceText == null) {
          ...
          if (match) {
            ...
            for (const currentPath of paths.concat(...opts.path).map(p => path.relative(baseDir, p))) {
              ...
              if (packageMains.has(packageName)) {
                // use cached
  ...
  for (let i = 0, k = argv.length; i < k; ++i) {
    ...
    if (sourceText == null) {
      ...
      if (sourceText != null) sourcePath = path;else sourcePath += extension.ext;
    } else {
  ...
  const zeroFilledMemory = opts.importMemory ? opts.zeroFilledMemory : false;
  const runPasses = [];
  if (opts.runPasses) {
  ...
  if (!opts.noEmit) {
    ...
    let hasStdout = false;
    let hasOutput = opts.textFile != null || opts.binaryFile != null || opts.jsFile != null || opts.tsdFile != null || opts.idlFile != null;
    // Write binary
    if (opts.binaryFile != null) {
      let basename = path.basename(opts.binaryFile);
      let sourceMapURL = opts.sourceMap != null ? opts.sourceMap.length ? opts.sourceMap : `./${basename}.map` : null;
      let wasm;
      stats.emitCount++;
      ...
      if (wasm.sourceMap != "") {
        if (opts.binaryFile.length) {
          ...
          writeFile(path.join(path.dirname(opts.binaryFile), path.basename(sourceMapURL)).replace(/^\.\//, ""), JSON.stringify(map), baseDir);
        } else {
    ...
    if (opts.textFile != null || !hasOutput) {
      ...
      if (opts.textFile != null && opts.textFile.length) {
        ...
        stats.emitTime += measure(() => {
          try {
            out = wastFormat ? module.emitText() : module.emitStackIR(true);
          } catch (e) {
            crash("emitText", e);
          }
        });
...
function checkDiagnostics(program, stderr, reportDiagnostic) {
  ...
  do {
    ...
    __pin(diagnosticPtr);
    if (stderr) {
      stderr.write(__getString(assemblyscript.formatDiagnostic(diagnosticPtr, stderr.isTTY, true)) + EOL + EOL);
    }
    if (reportDiagnostic) {
